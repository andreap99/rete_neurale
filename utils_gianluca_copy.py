...
def extract_time_windows(df, length=300, slide=None, freq=5, max_SOC=100, min_SOC=0, random_starting_point=True, verbose=False, overwrite=False, basepath='.', suffix='', random_state=None):
    ...
    for (soh, dc), temp_df in tqdm(df.groupby(level=[0, 1]), disable= not verbose):
        ...
        #argmax returns the position, max returns the value instead
        upper_cut = temp_df['SOC'].le(max_SOC).argmax()
        if length[0] < 10:
            lower_cut = temp_df['SOC'].ge(min_SOC).cumsum().max() - length[0]
        else:
            ...
            while True:
                ...
                if ending_point > temp_df.shape[0]:
                    ...
                    if starting_point >= upper_cut and ending_point <= lower_cut:   # then, keep the time window
                        out.write(f'Length = {temp_df.shape[0]}\t{temp_df.loc[-1].name[2]}')
                        out.write(f'1{temp_df.iloc[starting_point].name[2]}\n')
                        out.write(f'2{temp_df.iloc[ending_point].name[2]}\n')
                        out.write('---------------------\n')
                        idx = (temp_df.iloc[starting_point].name[2], temp_df.iloc[ending_point].name[2])
                        X.append(idx)
                        y.append(soh)
                    
                    if slide and ((starting_point + slide*freq) < lower_cut) and ((starting_point + slide*freq) < temp_df.shape[0]):
...
def ds_to_efc_planes(df, solver='ols', verbose=True, random_state=42):
    """
